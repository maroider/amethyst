    ...
    T: BindingTypes,
{
    ...
    mouse_last_position: Option<(f64, f64)>,
    ...
    T: BindingTypes,
{
            ...
            ControllerAxisMoved { which, axis, value } => {
                if let Some(controller_id) = self.controller_idx_to_id(which) {
                    self.controller_axes
                        ...
                        .map(|entry| entry.2 = value.into())
                        .unwrap_or_else(|| {
                            self.controller_axes
                                .push((controller_id, axis, value.into()));
                        });
        ...
        A: Hash + Eq + ?Sized,
    {
        self.bindings.axes.get(id).map(|a| match *a {
            ...
            Axis::MouseWheel { horizontal } => self.mouse_wheel_value(horizontal).into(),
        })
    }
